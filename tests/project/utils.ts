...
export type ChildClosedOptions = OptionsWithEncoding & {
    serialization?: false | 'json';
    timeout?: number;
};

export type ChildClosedPayload = {
    stdout: string;
    stderr: string;
    response?: any;
};
 ...
 */
export async function childClosed(
    child: ChildProcess,
    options?: ChildClosedOptions,
): Promise<ChildClosedPayload> {
    const deserialize = (output: string) =>
        options?.serialization === 'json' ? JSON.parse(output) : output;
    const collect = async (readable: Readable) =>
        getStream(readable || new PassThrough(), options);
...
export const watchStream = (
    ...
    const forMatch = async (regExp: RegExp) => {
        return await forCondition(
            (chunk: string) => chunk.match(regExp) != null,
        );
    };
