...
describe('FIXES', () => {
    it('should add custom attributes to root element', () => {
        ...
        createOrUpdateDynamicTheme(
            { ...DEFAULT_THEME, mode: FilterMode.light },
            null,
            false,
        );
        expect(
    ...
    it('should invert selectors', async () => {
        container.innerHTML = multiline('<div class="logo">Some logo</div>');
        ...
        expect(getComputedStyle(container.querySelector('.logo')).filter).toBe(
            'invert(1) hue-rotate(180deg) contrast(0.9)',
        );
    });
    ...
    it('should insert CSS', async () => {
        ...
        createOrUpdateDynamicTheme(DEFAULT_THEME, fixes, false);
        expect(getComputedStyle(container.querySelector('.text')).color).toBe(
            'rgb(255, 0, 0)',
        );
    });
    ...
    it('should ignore inline style', async () => {
        ...
        createOrUpdateDynamicTheme(DEFAULT_THEME, fixes, false);
        expect(
            getComputedStyle(container.querySelector('.text')).backgroundColor,
        ).toBe('rgb(128, 0, 128)');
    });
