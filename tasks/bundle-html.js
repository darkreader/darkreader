...
function html(platform, title, loader, stylesheet, compatibility) {
    return [
        ...
        compatibility && platform === PLATFORM.CHROMIUM_MV2
            ? '        <script src="compatibility.js" defer></script>'
            : null,
        '    </head>',
        '',
        loader
...
async function bundleHTML({ platforms, debug }) {
    ...
    for (const entry of copyEntries) {
        if (
            entry.platforms &&
            !entry.platforms.some((platform) => platforms[platform])
        ) {
            continue;
        }
...
const bundleHTMLTask = createTask('bundle-html', bundleHTML);
