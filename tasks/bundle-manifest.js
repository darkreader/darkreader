...
async function patchManifest(platform, debug, watch, test) {
    ...
    const manifestPatch =
        platform === PLATFORM.CHROMIUM_MV2
            ? {}
            : await readJSON(rootPath(`src/manifest-${platform}.json`));
    ...
    if (debug) {
        patched.version = '1';
        patched.description = `Debug build, platform: ${platform}, watch: ${
            watch ? 'yes' : 'no'
        }.`;
    }
...
const bundleManifestTask = createTask('bundle-manifest', manifests).addWatcher(
    ['src/manifest*.json'],
    async (changedFiles, _, buildPlatforms) => {
        const chrome = changedFiles.some((file) =>
        ...
        for (const platform of Object.values(PLATFORM)) {
            const changed =
                ...
                changedFiles.some((file) =>
                    file.endsWith(`manifest-${platform}.json`),
                );
