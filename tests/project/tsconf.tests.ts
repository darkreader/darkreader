...
describe('TypeScript project config', () => {
    it.each(tsProjects)(
        'file should parse and resolve correctly: $tsconfig',
        async ({ tsconfig }) => {
            ...
            // Fire
            const child = fork(tsc, ['--project', project, '--showConfig'], {
                silent: true,
                cwd,
            });

            const { response } = await childClosed(child, {
                serialization: 'json',
            });
    ...
    it.each(tsProjects)(
        'should compile without errors: $tsconfig',
        async ({ tsconfig }) => {
            ...
            // Fire
            const child = fork(tsc, ['--project', project, '--noEmit'], {
                silent: true,
                cwd,
            });
