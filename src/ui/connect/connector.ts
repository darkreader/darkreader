...
export default class Connector implements ExtensionActions {
    ...
    private async firefoxSendRequestWithResponse<T>(
        type: MessageTypeUItoBG,
        data?: string,
    ): Promise<T> {
        return new Promise<T>((resolve, reject) => {
    ...
    public async getData(): Promise<ExtensionData> {
        if (isFirefox) {
            return await this.firefoxSendRequestWithResponse<ExtensionData>(
                MessageTypeUItoBG.GET_DATA,
            );
        }
        return await this.sendRequest<ExtensionData>(
            MessageTypeUItoBG.GET_DATA,
        );
    }
