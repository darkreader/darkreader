...
function createValidator() {
    ...
    function validateProperty<T extends Record<string, unknown>>(
        ...
        errors.push(
            `Unexpected value for "${key as string}": ${JSON.stringify(obj[key])}`
        );
    ...
    function validateArray<T extends Record<string, unknown>, V>(
        ...
        if (wrongValues.size > 0) {
            errors.push(
                `Array "${key as string}" has wrong values: ${Array.from(wrongValues)
...
export function validateSettings(
    ...
    validateArray(settings, 'customThemes', (custom: CustomSiteConfig) => {
        if (
            !(
                isPlainObject(custom) && hasRequiredProperties(custom, ['url', 'theme'])
            )
        ) {
            return false;
        }
