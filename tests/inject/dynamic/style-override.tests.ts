...
describe('STYLE ELEMENTS', () => {
    it('should fill CSSStyleSheet with overriden rules', () => {
        ...
        expect((override.cssRules[0] as CSSStyleRule).selectorText).toBe(
            'body',
        );
        expect(
            (override.cssRules[0] as CSSStyleRule).style.getPropertyValue(
                'background-color',
            ),
        ).toBe('rgb(0, 0, 0)');
        expect((override.cssRules[1] as CSSStyleRule).selectorText).toBe('h1');
        expect(
            (override.cssRules[1] as CSSStyleRule).style.getPropertyValue(
                'color',
            ),
    ...
    it('should override User Agent style', async () => {
        ...
        expect(getComputedStyle(container.querySelector('span')).color).toBe(
            'rgb(255, 255, 255)',
        );
        expect(getComputedStyle(container.querySelector('a')).color).toBe(
            'rgb(51, 145, 255)',
        );
    });
    ...
    it('should override style with @import', async () => {
        container.innerHTML = multiline(
            '<style>',
            `    @import "data:text/css;utf8,${encodeURIComponent(
                'h1 { background: gray; }',
            )}";`,
            '    h1 strong { color: red; }',
            '</style>',
    ...
    it('should restore override', async () => {
        ...
        await timeout(0);
        expect(
            (style.nextSibling as HTMLStyleElement).classList.contains(
                'darkreader--sync',
            ),
        ).toBe(true);
        expect(getComputedStyle(container.querySelector('h1')).color).toBe(
    ...
    it('should react to updated style', async () => {
        ...
        await timeout(0);
        expect(
            (style.nextSibling as HTMLStyleElement).sheet.cssRules[0].cssText,
        ).toBe('body { background-color: rgb(50, 0, 9); }');
        expect(getComputedStyle(container.querySelector('h1')).color).toBe(
            'rgb(141, 141, 141)',
        );
        expect(
    ...
    it('should react on style text change', async () => {
        ...
        document.querySelector('.testcase-style').textContent =
    ...
    it('should react to a new style', async () => {
        ...
        await timeout(0);
        const newStyle: HTMLStyleElement =
            document.querySelector('.testcase-style');
