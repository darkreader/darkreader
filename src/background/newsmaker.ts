...
export default class Newsmaker {
    private static readonly UPDATE_INTERVAL = getDurationInMinutes({
        hours: 4,
    });
    ...
    private static init() {
        ...
        Newsmaker.stateManager = new StateManager<NewsmakerState>(
    ...
    public static subscribe(): void {
        ...
        if (
            Newsmaker.latestTimestamp === null ||
            Newsmaker.latestTimestamp + Newsmaker.UPDATE_INTERVAL < Date.now()
        ) {
            Newsmaker.updateNews();
        }
