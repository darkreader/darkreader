...
import {
    ...
    stubBackgroundFetchResponse,
    stubChromeRuntimeMessage,
} from '../support/background-stub';
...
describe('LINK STYLES', () => {
    it('should override same-origin link', async () => {
        createStyleLink(
            `data:text/css;utf8,${encodeURIComponent(
                multiline(
                    'h1 { background: gray; }',
                    'h1 strong { color: red; }',
                ),
    ...
    it('should override cross-origin link', async () => {
        createCorsLink(
            multiline('h1 { background: gray; }', 'h1 strong { color: red; }'),
        );
        container.innerHTML = multiline(
    ...
    it('should override cross-origin imports in linked CSS', async () => {
        ...
        createCorsLink(
            multiline(`@import "${importedURL}";`, 'h1 strong { color: red; }'),
        );
        container.innerHTML = multiline(
    ...
    it('should remove manager from disabled link', async () => {
        ...
        await timeout(50);
        expect(
            selectTestStyleLink().nextElementSibling.classList.contains(
                isSafari ? 'darkreader--cors' : 'darkreader--sync',
            ),
        ).toBe(true);
        link.disabled = true;
        await timeout(0);
    ...
    it('should handle styles with invalid url(...)', async () => {
        const importedCSS =
            'h1 { background-image: url("freecookies:3https://example.com"); background-color: gray; }';
        const importedURL = getCSSEchoURL(importedCSS);
        stubBackgroundFetchResponse(importedURL, importedCSS);
