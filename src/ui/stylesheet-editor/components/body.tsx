...
export default function Body({data, tab, actions}: BodyProps) {
    ...
    function onTextRender(node) {
        textNode = node;
        textNode.value = (custom ? custom.appearance.stylesheet : data.settings.appearance.stylesheet) || '';
    ...
    function applyStyleSheet(css: string) {
        if (custom) {
            custom.appearance = {...custom.appearance, ...{stylesheet: css}};
            actions.changeSettings({customAppearance: data.settings.customAppearance});
        } else {
            actions.setAppearance({stylesheet: css});
        }
    }
