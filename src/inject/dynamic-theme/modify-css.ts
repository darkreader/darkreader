...
function getModifiedScrollbarStyle(theme: Theme) {
    ...
    if (theme.scrollbarColor === 'auto') {
        ...
        const lighten = (lighter: number) => ({
            ...hsl,
            l: clamp(hsl.l + lighter, 0, 1),
        });
        const darken = (darker: number) => ({
            ...hsl,
    ...
    try {
        ...
        const getGradientModifier = (gradient: parsedGradient) => {
            ...
            const parts = getMatches(partsRegex, match, 1).map((part) => {
                ...
                if (rgb) {
                    return (filter: FilterConfig) =>
                        `${modifyGradientColor(rgb!, filter)} ${part.substring(
                            space + 1,
                        )}`;
                }
...
export function getShadowModifierWithInfo(
    ...
    try {
        ...
        const modifiers = colorMatches.map((match, i) => {
            ...
            return (filter: FilterConfig) =>
                `${value.substring(prefixIndex, matchIndex)}${modifyShadowColor(
                    rgb,
                    filter,
                )}${
