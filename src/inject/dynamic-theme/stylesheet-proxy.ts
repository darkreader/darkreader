export function injectProxy(
    ...
    function proxyDocumentStyleSheets() {
        const getCurrentValue = () => {
            const docSheets: StyleSheetList =
                documentStyleSheetsDescriptor!.get!.call(this);
            ...
            const filteredSheets = [...docSheets].filter(
                (styleSheet) =>
                    ...
                    !(
                        (
                            styleSheet.ownerNode as Exclude<
                                typeof styleSheet.ownerNode,
                        ...
                        (
                            styleSheet.ownerNode as Exclude<
                                ...
                                ProcessingInstruction
                            >
                        ).classList.contains('darkreader')
                    ),
            );

            (filteredSheets as unknown as StyleSheetList).item = (
                item: number,
            ) => filteredSheets[item];
    ...
    function proxyCustomElementRegistryDefine(
        name: string,
        constructor: any,
        options: any,
    ) {
    ...
    function proxyGetElementsByTagName(
        tagName: string,
        ...
        const getCurrentElementValue = () => {
            const elements: NodeListOf<HTMLElement> =
                getElementsByTagNameDescriptor!.value.call(this, tagName);
    ...
    function proxyChildNodes(): NodeListOf<ChildNode> {
        const childNodes: NodeListOf<ChildNode> =
            childNodesDescriptor!.get!.call(this);

        return Object.setPrototypeOf(
            [...childNodes].filter((element: ChildNode) => {
                return (
                    !(element as HTMLElement).classList ||
                    !(element as HTMLElement).classList.contains('darkreader')
                );
            }),
            NodeList.prototype,
        );
